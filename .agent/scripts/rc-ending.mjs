...
//
// READ `result.ok` FROM THAT SECOND COMMAND. shot.mjs exits 0 even when the page
// script it ran reported a failure (#150), so the exit code of the replay says
// only that Chrome ran; `ok` is what says the game was played.
//
...
//
// Every return carries `ok`, and the caller has to read it: shot.mjs exits 0
// even when the script it ran reported a failure (#150). This script has already
// been silently green once — the mode picker landed in front of the board and it
// answered `{"error":"board not found"}`, zero turns played, exit code 0 — so
// `ok` is false unless the replay actually reached the board, played every turn,
// and arrived at a finished game.
...
// Read \`result.ok\` — shot.mjs exits 0 whatever this returns (#150).
(async () => {
