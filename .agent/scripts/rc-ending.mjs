...
// script for shot.mjs, which walks the mode picker into the hotseat game and
// replays that turn list by CLICKING THE REAL BOARD, then reads the rendered
// result back out of the DOM. Nothing is stubbed: the engine picks the game, the
// browser plays it, and what comes back is what a player would have seen.
//
...
//
// THAT SECOND COMMAND IS A GATE. The page script reports `result.ok`, and
// shot.mjs exits 3 — printing the whole result — when it is false (#150), so a
// replay that did not play the game fails the command rather than reporting it.
//
...
//
// Every return carries `ok`, and that is what fails the command: shot.mjs exits
// 3 on a falsy one (#150). This script has already been silently green once —
// the mode picker landed in front of the board and it answered
// `{"error":"board not found"}`, zero turns played, and shot.mjs exited 0
// anyway, which is the hole #150 closed — so `ok` is false unless the replay
// actually reached the board, played every turn, and arrived at a finished game.
...
// \`result.ok\` is the verdict — shot.mjs exits 3 when it is false (#150).
(async () => {
  ...
  // The front door. \`/games/race-condition\` opens on the mode picker, not on a
  // board — Same device / Play online / I have a code — so the hotseat game has
  // to be asked for before there is anything to click.
  //
  // Waiting for the BUTTON is not enough. The page is prerendered, so the markup
  // is there long before React owns it, and a click on an unhydrated button does
  // nothing at all: no handler, no error, no board. That is a false green of the
  // same family as the one this prelude exists to fix, so wait for the props
  // React attaches to the node on hydration instead of for the node itself.
  const hydrated = (el) =>
    ...
    for (let i = 0; i < 100; i += 1) {
      choice =
        [...document.querySelectorAll("button")].find((b) =>
          /Same device/.test(b.textContent)
        ) ?? null;

      if (hydrated(choice)) break;
      await sleep(100);
    }

    if (!hydrated(choice)) return fail("the mode picker never hydrated");

    choice.click();

    for (let i = 0; i < 60 && cells().length !== 16; i += 1) await sleep(50);
    await sleep(300);
  }

  if (cells().length !== 16)
    return fail("board not found after picking Same device");
  ...
  // ANCHOR. Every ending this script generates is a finished game, so the
  // endgame panel — "Play again" — has to be on screen. If the clicks landed on
  // a board that was not listening, the turn loop still runs to the end and the
  // report still comes back full of plausible-looking nulls; this is what makes
  // that read as a failure rather than as a result.
  const finished = playAgain() !== null;

  return {
    ok: played === TURNS.length && finished,
    error: finished
      ? played === TURNS.length
        ? null
        : \`only \${played} of \${TURNS.length} turns were played\`
      : "the game never finished — the clicks did not reach the board",
    turnsPlayed: played,
    banner: {
