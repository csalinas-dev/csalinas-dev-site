...
- `/games/infinite-tic-tac-toe` is a two-player hotseat tic-tac-toe variant in `src/app/(pages)/games/infinite-tic-tac-toe/`. Each player keeps at most three marks (`MAX_MARKS`); placing a fourth clears **that player's** oldest one, tracked as a per-player queue in `state.history`. State is a plain `useReducer` with no persistence or server actions.
