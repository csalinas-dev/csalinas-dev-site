...
- `src/content/posts/index.js` is the hand-maintained registry both `/blog` and `generateStaticParams` read. Adding a post is a directory plus one `import` line there. It is a list rather than an `fs` glob so posts are build-time ESM imports: no runtime markdown compilation, the whole blog prerenders, and a bad `meta` throws during `npm run build` instead of at request time.
