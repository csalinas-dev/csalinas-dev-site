import { CELLS, MAX_PLAYERS, MIN_PLAYERS, SIZE } from "./constants";
 ...
 * A fresh board.
 *
 * Unlike `playTurn`, this throws: a bad player list is a programming error in
 * the lobby, not a turn a player can take, and it must not produce a half-valid
 * board the rest of the engine then has to defend against.
 *
...
export const createState = ({ slots, first } = {}) => {
  if (!Array.isArray(slots) || !slots.every(isSlot)) {
    throw new Error(
      "Race Condition: slots must be an array of non-empty strings or integers"
    );
  }

  if (slots.length < MIN_PLAYERS || slots.length > MAX_PLAYERS) {
    throw new Error(
      `Race Condition: a game needs exactly ${MAX_PLAYERS} players, got ${slots.length}`
    );
  }

  if (new Set(slots).size !== slots.length) {
    throw new Error("Race Condition: slots must be unique");
  }

  if (first !== undefined && !slots.includes(first)) {
    throw new Error("Race Condition: `first` must be one of the slots");
  }

  return {
    size: SIZE,
    // Row-major and flat: index = row * 4 + col, row 0 on TOP. See the indexing
    // convention at the top of `board.js`.
...
/**
 * The same two players, back at turn one — the "play again" button.
 *
 * The winner opens the next game, which is the convention every one of these
 * games uses. A draw has no winner to hand the first turn to, so it goes back to
 * join order.
 *
